...
class TestSleeveReturnsEmission:
    ...
    def test_sleeve_returns_includes_sr3_calendar_spread_carry(self, tmp_path):
        """Precomputed sr3_calendar_spread_carry returns are written as sleeve column when provided."""
        dates = pd.bdate_range("2023-01-02", periods=20, freq="B")
        returns_df = pd.DataFrame(
            np.random.randn(len(dates), 2).astype(np.float64) * 0.01,
            index=dates,
        ...
        sr3_calendar_spread_carry_returns = pd.Series(
            np.random.randn(len(dates)).astype(np.float64) * 0.005, index=dates
        )
        equity = (1 + returns_df.sum(axis=1)).cumprod()
        equity.iloc[0] = 1.0
        weights_panel = pd.DataFrame(
        ...
        sleeve_path = tmp_path / run_id / "sleeve_returns.csv"
        assert sleeve_path.exists(), "sleeve_returns.csv should be written when sr3_calendar_spread_carry returns provided"
        sleeve_df = pd.read_csv(sleeve_path, index_col=0, parse_dates=True)
        assert "sr3_calendar_spread_carry" in sleeve_df.columns
