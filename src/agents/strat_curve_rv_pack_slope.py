"""
SR3 Curve RV Pack Slope Momentum Atomic Sleeve

Atomic sleeve for Pack Slope (front vs back) momentum strategy.
...
from src.strategies.rates_curve_rv.sr3_curve_rv_momentum import compute_pack_slope_momentum_phase1

logger = logging.getLogger(__name__)


class SR3CurveRVPackSlopeAtomic:
    """
    Atomic sleeve for SR3 Curve RV Pack Slope Momentum.
    
    Computes Pack Slope (front vs back) momentum signals and returns portfolio returns
    ...
    def __init__(
        ...
        """
        Initialize Pack Slope atomic sleeve.
        ...
        logger.info(
            f"[SR3CurveRVPackSlopeAtomic] Initialized with "
    ...
    def compute_returns(
        ...
        """
        Compute portfolio returns for Pack Slope momentum.
        ...
        logger.info(f"[SR3CurveRVPackSlopeAtomic] Computing returns from {start} to {end}")
        
        # Use Phase-1 computation
        result = compute_pack_slope_momentum_phase1(
            market=market,
            start_date=start,
            end_date=end,
