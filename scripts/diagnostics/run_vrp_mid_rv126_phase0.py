#!/usr/bin/env python3
"""
VRP-Mid (RV126) Phase-0 Signal Test Script

This script performs Phase-0 signal sanity tests for VRP-Mid using RV126:
- Economic spec: Mid-horizon implied vs realized volatility (VIX - RV126)
- Toy rule: Short VX when VIX > RV126, else flat
- Tests two variants: VX2, VX3
...
- Pass criteria: Sharpe ≥ 0.10 and non-catastrophic MaxDD; otherwise FAIL and PARK

Usage:
    python scripts/diagnostics/run_vrp_mid_rv126_phase0.py
    python scripts/diagnostics/run_vrp_mid_rv126_phase0.py --start 2020-01-01 --end 2025-10-31
"""
...
def generate_phase0_plots(df: pd.DataFrame, output_dir: Path, tenor: str):
    ...
    ax.plot(df['date'], equity, label=f'VRP-Mid (RV126) Phase-0 - {tenor}', linewidth=1.5)
    ax.set_xlabel('Date')
    ax.set_ylabel('Equity')
    ax.set_title(f'VRP-Mid (RV126) Phase-0: Equity Curve ({tenor})')
    ax.legend()
    ...
    # Plot 2: Spread time series (VIX - RV126)
    fig, ax = plt.subplots(figsize=(14, 6))
    
    # Plot spread
    ax.plot(df['date'], df['spread'], label='Spread (VIX - RV126)', alpha=0.7, color='blue', linewidth=1.5)
    ...
    if active_mask.any():
        ax.fill_between(
            ...
            alpha=0.2,
            color='red',
            label='Short Active (VIX > RV126)'
        )
    
    ax.set_xlabel('Date')
    ax.set_ylabel('Spread (vol points)')
    ax.set_title(f'VRP-Mid Spread Over Time (VIX - RV126) with Active Signal Shading ({tenor})')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title(f'VRP-Mid (RV126) Phase-0: Return Distribution ({tenor})')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def run_vrp_mid_rv126_variant(
    con: duckdb.DuckDBPyConnection,
    start: str,
    end: str,
    output_dir: Path,
    tenor: int,  # 2 or 3
    df_base: pd.DataFrame,  # Pre-loaded VIX, RV126, VX curve data
) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        tenor: VX tenor (2 or 3)
        df_base: DataFrame with VIX, RV126, and VX curve data
    ...
    logger.info("\n" + "=" * 80)
    logger.info(f"VRP-MID (RV126) PHASE-0 SIGNAL TEST - {tenor_name}")
    logger.info("=" * 80)
    logger.info("Economic spec: Mid-horizon implied vs realized volatility (VIX - RV126)")
    logger.info(f"Toy rule: Short {tenor_name} when VIX > RV126, else flat")
    ...
    df = df.dropna(subset=['vix', 'rv126', vx_col]).sort_values('date').copy()
    
    if df.empty:
        logger.error(f"  ERROR: No overlapping data for {tenor_name} after dropna")
        raise ValueError(f"No overlapping VIX/RV126/{vx_col} data available")
    
    logger.info(f"  Loaded {len(df)} days of data")
    logger.info(f"  VIX: mean={df['vix'].mean():.2f}, std={df['vix'].std():.2f}")
    logger.info(f"  RV126: mean={df['rv126'].mean():.2f}, std={df['rv126'].std():.2f}")
    ...
    initial_rows = len(df_base.dropna(subset=['vix', 'rv126', vx_col]))
    ...
    df[['date', 'vix', 'rv126', vx_col, 'spread', 'signal', 'position', 'vx_return', 'pnl']].to_parquet(
        save_dir / f'vrp_mid_rv126_{tenor_name.lower()}_phase0_timeseries.parquet', index=False
    )
    df[['date', 'vix', 'rv126', vx_col, 'spread', 'signal', 'position', 'vx_return', 'pnl']].to_csv(
        save_dir / f'vrp_mid_rv126_{tenor_name.lower()}_phase0_timeseries.csv', index=False
    )
    ...
    metrics_to_save = {
        ...
        'description': f'VRP-Mid (RV126) Phase-0 signal test: Short {tenor_name} when VIX > RV126, else flat. Tests mid-horizon implied vs realized volatility premium.',
        'rule_type': 'short_only',
        'rationale': 'Mid-horizon implied volatility (VIX) vs mid-horizon realized volatility (RV126) should contain a tradable volatility risk premium.',
    ...
    phase_index_dir = Path(f"reports/phase_index/vrp/mid_rv126_{tenor_name.lower()}")
    phase_index_dir.mkdir(parents=True, exist_ok=True)
    
    phase0_file = phase_index_dir / "phase0.txt"
    with open(phase0_file, 'w') as f:
        f.write(f"reports/sanity_checks/vrp/mid_rv126_{tenor_name.lower()}/latest/\n")
    
    logger.info(f"  Registered in: {phase0_file}")
    
    # Print summary
    print("\n" + "=" * 80)
    print(f"VRP-MID (RV126) PHASE-0 SIGNAL TEST SUMMARY - {tenor_name}")
    print("=" * 80)
    print(f"Rule: Short {tenor_name} when VIX > RV126, else flat")
    print(f"Economic thesis: Mid-horizon implied vs realized volatility premium")
    print(f"\nMetrics:")
    ...
    if not pd.isna(sharpe):
        if sharpe >= 0.10:
            print(f"  [PASS] Sharpe >= 0.10: {sharpe:.4f} (PASS)")
        else:
    ...
    if not pd.isna(maxdd):
        ...
        print(f"  [FAIL] MaxDD could not be computed (FAIL)")
...
def run_vrp_mid_rv126_phase0(
    ...
    """
    Run VRP-Mid (RV126) Phase-0 signal test for two variants (VX2, VX3).
    ...
    """
    logger.info("\n" + "=" * 80)
    logger.info("VRP-MID (RV126) PHASE-0 SIGNAL TEST")
    logger.info("=" * 80)
    logger.info("Economic spec: Mid-horizon implied vs realized volatility (VIX - RV126)")
    logger.info("Toy rule: Short VX when VIX > RV126, else flat")
    logger.info("Testing two variants: VX2, VX3")
    ...
    # 2) Load RV126 from ES returns (using MarketData, consistent with VRP-Core/Alt)
    logger.info("\n[2/5] Loading RV126 (126-day realized volatility) from ES returns...")
    import yaml
    ...
    try:
        ...
        # Compute RV126 from ES returns
        # RV126 = rolling std of daily log returns * sqrt(252) * 100 (vol points)
        rv126 = es_returns[es_symbol].rolling(
            window=126,
            min_periods=126
        ...
        logger.info(f"  Computed {len(df_rv)} RV126 rows")
        logger.info(f"  RV126: mean={df_rv['rv126'].mean():.2f}, std={df_rv['rv126'].std():.2f}")
        
    finally:
        market.close()
    
    # 3) Load VX curve
    logger.info("\n[3/5] Loading VX curve data (VX2, VX3)...")
    ...
    # 5) Run two variants (VX2, VX3)
    logger.info("\n[5/5] Running Phase-0 tests for VX2 and VX3 variants...")
    ...
    for tenor in tenors:
        tenor_name = f"VX{tenor}"
        output_dir = base_output_dir / f"mid_rv126_{tenor_name.lower()}"
        output_dir.mkdir(parents=True, exist_ok=True)
        
        try:
            result = run_vrp_mid_rv126_variant(
    ...
    compare_dir = base_output_dir / "mid_rv126_compare" / "latest"
    ...
    print("\n" + "=" * 80)
    print("VRP-MID (RV126) PHASE-0 COMPARISON SUMMARY")
    print("=" * 80)
    print(f"Window: {start} to {end}")
    print(f"\nVariant Comparison:")
    for tenor_name in ['VX2', 'VX3']:
...
def main():
    parser = argparse.ArgumentParser(
        description="VRP-Mid (RV126) Phase-0 Signal Test (VX2/VX3)",
...
Examples:
  ...
  python scripts/diagnostics/run_vrp_mid_rv126_phase0.py --start 2004-01-01 --end 2025-10-31
        """
    )
    ...
    try:
        ...
        try:
            # Run VRP-Mid Phase-0 signal test (VX2, VX3)
            summary = run_vrp_mid_rv126_phase0(
        ...
        print(f"  - mid_rv126_vx2/latest/   (VX2 variant)")
        print(f"  - mid_rv126_vx3/latest/   (VX3 variant)")
        print(f"  - mid_rv126_compare/latest/summary.json   (comparison summary)")
        print(f"\nPhase index entries:")
        print(f"  - reports/phase_index/vrp/mid_rv126_vx2/phase0.txt")
        print(f"  - reports/phase_index/vrp/mid_rv126_vx3/phase0.txt")
