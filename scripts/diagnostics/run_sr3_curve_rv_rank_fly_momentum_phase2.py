#!/usr/bin/env python3
"""
Phase-2 Diagnostics Script for SR3 Curve RV Rank Fly Momentum Integration.

Compares baseline (Core v8) vs Rank Fly Momentum-enhanced portfolio (Core v9).

This script:
1. Runs baseline strategy (Core v8) via run_strategy.py
2. Runs variant strategy (Core v9 + Rank Fly) via run_strategy.py
...
Usage:
    python scripts/diagnostics/run_sr3_curve_rv_rank_fly_momentum_phase2.py
    python scripts/diagnostics/run_sr3_curve_rv_rank_fly_momentum_phase2.py --start 2020-01-01 --end 2025-10-31 --core-v8-run-id <id>
"""
...
VARIANT_PROFILE = "core_v9_trend_csmom_vrp_core_convergence_vrp_alt_vx_carry_sr3_curverv_rankfly_no_macro"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SR3 Curve RV Rank Fly Momentum Phase-2 Diagnostics")
    ...
    logger.info("=" * 80)
    logger.info("SR3 Curve RV Rank Fly Momentum Phase-2 Diagnostics")
    logger.info("=" * 80)
    ...
    if args.variant_run_id:
        ...
        variant_run_id = f"core_v9_rankfly_phase2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # Step 8: Save results
    output_dir = Path("reports/phase_index/rates_curve_rv/sr3_curve_rv_rank_fly_2_6_10_momentum/phase2")
    ...
    update_phase_index(
        ...
        sleeve_name="sr3_curve_rv_rank_fly_2_6_10_momentum",
        phase="phase2",
        run_id=variant_run_id
    )
