#!/usr/bin/env python3
"""
Carry Phase-1.1 Diagnostic Script

Runs Phase-1.1 backtest (asset-class risk parity) and generates analysis.

Usage:
    python scripts/diagnostics/run_carry_phase1_1_v1.py
    python scripts/diagnostics/run_carry_phase1_1_v1.py --start 2020-01-01 --end 2025-10-31
"""
...
def analyze_phase1_1_results(run_id: str) -> dict:
    """Analyze Phase-1.1 backtest results."""
    ...
    # Compute statistics
    stats = compute_summary_stats(returns, name="Carry Meta v1 (Phase-1.1)")
...
def run_phase1_1_backtest(start: str, end: str, config_path: str) -> str:
    """Run Phase-1.1 backtest and return run_id."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_id = f"carry_phase1_1_v1_{timestamp}"
    ...
    logger.info("=" * 80)
    logger.info("CARRY META-SLEEVE V1 — PHASE-1.1 BACKTEST")
    logger.info("=" * 80)
...
def main():
    """Run Phase-1.1 diagnostic."""
    parser = argparse.ArgumentParser(description="Carry Phase-1.1 Diagnostic")
    parser.add_argument(
        "--start",
        type=str,
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/carry_phase1_1_v1.yaml",
        help="Config file path"
    )
    
    args = parser.parse_args()
    
    # Run Phase-1.1 backtest
    run_id = run_phase1_1_backtest(args.start, args.end, args.config)
    
    # Phase-1.1 Compliance Check
    run_dir = Path("reports/runs") / run_id
    ...
    if meta_path.exists():
        with open(meta_path) as f:
            meta = json.load(f)
        logger.info("\n" + "=" * 80)
        logger.info("PHASE-1.1 COMPLIANCE CHECK")
        logger.info("=" * 80)
        ...
        logger.info(f"Carry Phase: {phase} (required: 1.1)")
        ...
        if compliance_pass:
            logger.info("Phase-1.1 Compliance: PASSED")
        else:
            logger.warning("Phase-1.1 Compliance: FAILED - Check config flags above")
        logger.info("=" * 80)
    
    # Analyze results
    logger.info("\n" + "=" * 80)
    logger.info("CARRY META-SLEEVE V1 — PHASE-1.1 ANALYSIS")
    logger.info("=" * 80)
    
    summary = analyze_phase1_1_results(run_id)
    ...
    for window_name, window_data in summary.get('stress_windows', {}).items():
        ...
        logger.info(f"{window_name}: {cumret:.2%} {'PASS' if acceptable else 'FAIL'}")
    ...
    for asset_class, contrib in summary.get('asset_class_contributions', {}).items():
        ...
        logger.info(f"{asset_class.upper()}: CumRet={cumret:.2%}, Sharpe={sharpe:.3f}")
    
    # Pass criteria
    logger.info("\n" + "=" * 80)
    logger.info("PHASE-1.1 PASS CRITERIA")
    logger.info("=" * 80)
    ...
    if pc['overall_pass']:
        logger.info("\n" + "=" * 80)
        logger.info("PHASE-1.1 PASSED (RECOMMENDED)")
        logger.info("=" * 80)
    elif pc['conditional_pass']:
        logger.info("\n" + "=" * 80)
        logger.info("PHASE-1.1 CONDITIONAL PASS")
        logger.info("=" * 80)
    else:
        logger.info("\n" + "=" * 80)
        logger.info("PHASE-1.1 FAILED")
        logger.info("=" * 80)
    
    # Save summary
    summary_path = run_dir / "phase1_1_analysis_summary.json"
