#!/usr/bin/env python3
"""
Phase-2 Diagnostics Script for VRP-Alt Integration.

Compares baseline (core_v6_trend_csmom_vrp_core_convergence_no_macro) vs VRP-Alt-enhanced (core_v6_trend_csmom_vrp_core_convergence_vrp_alt_no_macro).

Note on Phase-0 MaxDD:
VRP-Alt (VIX – RV5) showed borderline economic edge in Phase-0 (Sharpe ≈ 0.10) with catastrophic MaxDD (~–94%), 
similar to VRP-Core Phase-0. This is expected for raw, unscaled short-vol signals.
Phase-1 engineering (z-scoring + vol targeting + short-only constraint) transformed VRP-Alt into a very stable sleeve 
(Sharpe ≈ 0.91, MaxDD ≈ –2.0%).
Phase-2 now evaluates whether adding this engineered sleeve to the Core v6 portfolio improves or maintains 
portfolio-level Sharpe and drawdowns without introducing crisis-period pathologies.

This script:
1. Runs baseline strategy (Trend + CSMOM + VRP-Core + VRP-Convergence)
2. Runs VRP-Alt-enhanced strategy (Trend + CSMOM + VRP-Core + VRP-Convergence + VRP-Alt)
3. Compares portfolio metrics (Sharpe, CAGR, Vol, MaxDD, HitRate)
4. Analyzes crisis-period performance
5. Computes sleeve-level correlations including VRP-Alt
6. Saves comparison outputs and registers in phase index

Usage:
    python scripts/diagnostics/run_core_v6_trend_csmom_vrp_core_convergence_vrp_alt_phase2.py
    python scripts/diagnostics/run_core_v6_trend_csmom_vrp_core_convergence_vrp_alt_phase2.py --start 2020-01-01 --end 2025-10-31
"""
...
def compute_sleeve_returns(profile_name: str, start_date: str, end_date: str, market: MarketData) -> Dict[str, pd.Series]:
    ...
    # Compute VRP-Alt sleeve returns
    if strategies_cfg.get("vrp_alt_meta", {}).get("enabled", False):
        logger.info("  Computing VRP-Alt sleeve returns...")
        vrp_alt_params = strategies_cfg.get("vrp_alt_meta", {}).get("params", {})
        # Get DB path
        data_config_path = Path("configs/data.yaml")
        db_path = None
        if data_config_path.exists():
        ...
        # Run backtest for VRP-Alt sleeve only
        combined = CombinedStrategy(
            strategies={"vrp_alt_meta": vrp_alt},
            weights={"vrp_alt_meta": 1.0},
            feature_service=feature_service
        )
        components = {
            'strategy': combined,
        ...
        results = exec_sim.run(market, start_date, end_date, components)
        vrp_alt_equity = results['equity_curve']
        vrp_alt_returns = vrp_alt_equity.pct_change().dropna()
        sleeve_returns['vrp_alt'] = vrp_alt_returns
...
def main():
    parser = argparse.ArgumentParser(
        description="Phase-2 Diagnostics: VRP-Alt Integration",
...
Examples:
  ...
  python scripts/diagnostics/run_core_v6_trend_csmom_vrp_core_convergence_vrp_alt_phase2.py --start 2020-01-01 --end 2025-10-31
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory (default: data/diagnostics/phase2/core_v6_trend_csmom_vrp_core_convergence_vrp_alt)"
    )
    parser.add_argument(
        "--baseline_profile",
        type=str,
        default="core_v6_trend_csmom_vrp_core_convergence_no_macro",
        help="Baseline profile name (default: core_v6_trend_csmom_vrp_core_convergence_no_macro)"
    )
    parser.add_argument(
        "--variant_profile",
        type=str,
        default="core_v6_trend_csmom_vrp_core_convergence_vrp_alt_no_macro",
        help="Variant profile name (default: core_v6_trend_csmom_vrp_core_convergence_vrp_alt_no_macro)"
    )
    ...
    try:
        logger.info("=" * 80)
        logger.info("VRP-ALT PHASE-2 DIAGNOSTICS")
        logger.info("=" * 80)
        logger.info(f"Start date: {args.start}")
        logger.info(f"End date: {args.end}")
        logger.info(f"Baseline profile: {args.baseline_profile}")
        logger.info(f"Variant profile: {args.variant_profile}")
        ...
        if args.output_dir:
            ...
            output_dir = Path(f"data/diagnostics/phase2/core_v6_trend_csmom_vrp_core_convergence_vrp_alt/{timestamp}")
        ...
        # Generate run IDs
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        baseline_run_id = f"core_v6_baseline_phase2_{timestamp}"
        variant_run_id = f"core_v6_vrp_alt_phase2_{timestamp}"
        
        # 1) Run baseline (core_v6)
        logger.info("\n" + "=" * 80)
        logger.info("[1/4] Running baseline: core_v6_trend_csmom_vrp_core_convergence_no_macro")
        logger.info("=" * 80)
        ...
        # 2) Run VRP-Alt-enhanced (core_v6 + VRP-Alt)
        logger.info("\n" + "=" * 80)
        logger.info("[2/4] Running VRP-Alt-enhanced: core_v6_trend_csmom_vrp_core_convergence_vrp_alt_no_macro")
        logger.info("=" * 80)
        run_strategy_profile(
            profile_name=args.variant_profile,
        ...
        market = MarketData()
        try:
            # Compute sleeve returns for variant profile (core_v6 + VRP-Alt)
            ...
            if len(corr_data) > 0:
                ...
                if len(corr_df) > 0:
                    ...
                    # Extract VRP-Alt correlations
                    if 'vrp_alt' in corr_df.columns:
                        if 'trend' in corr_df.columns:
                            sleeve_corr_summary['corr_vrp_alt_vs_trend'] = float(corr_matrix.loc['vrp_alt', 'trend'])
                        if 'csmom' in corr_df.columns:
                            sleeve_corr_summary['corr_vrp_alt_vs_csmom'] = float(corr_matrix.loc['vrp_alt', 'csmom'])
                        if 'vrp_core' in corr_df.columns:
                            sleeve_corr_summary['corr_vrp_alt_vs_vrp_core'] = float(corr_matrix.loc['vrp_alt', 'vrp_core'])
                        if 'vrp_convergence' in corr_df.columns:
                            sleeve_corr_summary['corr_vrp_alt_vs_vrp_convergence'] = float(corr_matrix.loc['vrp_alt', 'vrp_convergence'])
                        if 'baseline_portfolio' in corr_df.columns:
                            sleeve_corr_summary['corr_vrp_alt_vs_baseline_portfolio'] = float(corr_matrix.loc['vrp_alt', 'baseline_portfolio'])
                        if 'variant_portfolio' in corr_df.columns:
                            sleeve_corr_summary['corr_vrp_alt_vs_variant_portfolio'] = float(corr_matrix.loc['vrp_alt', 'variant_portfolio'])
            
            # Log sleeve correlations
            if sleeve_corr_summary:
                logger.info("\n=== Sleeve-Level Correlations (VRP-Alt Phase-2) ===")
                if 'corr_vrp_alt_vs_trend' in sleeve_corr_summary:
                    logger.info("corr(VRP-Alt, Trend): %.4f", sleeve_corr_summary['corr_vrp_alt_vs_trend'])
                if 'corr_vrp_alt_vs_csmom' in sleeve_corr_summary:
                    logger.info("corr(VRP-Alt, CSMOM): %.4f", sleeve_corr_summary['corr_vrp_alt_vs_csmom'])
                if 'corr_vrp_alt_vs_vrp_core' in sleeve_corr_summary:
                    logger.info("corr(VRP-Alt, VRP-Core): %.4f", sleeve_corr_summary['corr_vrp_alt_vs_vrp_core'])
                if 'corr_vrp_alt_vs_vrp_convergence' in sleeve_corr_summary:
                    logger.info("corr(VRP-Alt, VRP-Convergence): %.4f", sleeve_corr_summary['corr_vrp_alt_vs_vrp_convergence'])
                if 'corr_vrp_alt_vs_baseline_portfolio' in sleeve_corr_summary:
                    logger.info("corr(VRP-Alt, Baseline Portfolio): %.4f", sleeve_corr_summary['corr_vrp_alt_vs_baseline_portfolio'])
                if 'corr_vrp_alt_vs_variant_portfolio' in sleeve_corr_summary:
                    logger.info("corr(VRP-Alt, Variant Portfolio): %.4f", sleeve_corr_summary['corr_vrp_alt_vs_variant_portfolio'])
        ...
        with open(output_dir / 'crisis_summary.json', 'w') as f:
        ...
        try:
            ...
            ax.plot(baseline_equity.index, baseline_equity.values, label='Baseline (core_v6)', linewidth=1.5)
            ax.plot(variant_equity.index, variant_equity.values, label='Variant (core_v6 + VRP-Alt)', linewidth=1.5)
            ax.set_xlabel('Date')
            ...
            fig, ax = plt.subplots(figsize=(14, 6))
            ax.plot(baseline_dd.index, baseline_dd.values, label='Baseline (core_v6)', linewidth=1.5)
            ax.plot(variant_dd.index, variant_dd.values, label='Variant (core_v6 + VRP-Alt)', linewidth=1.5)
            ax.set_xlabel('Date')
        ...
        phase2_file = phase_index_dir / "phase2_core_v6_trend_csmom_vrp_core_convergence_vrp_alt.txt"
        
        # Phase-2 Pass Criteria Check
        sharpe_diff = diff_metrics.get('sharpe_diff', float('-inf'))
        maxdd_diff = diff_metrics.get('maxdd_diff', float('inf'))
        corr_vrp_core = sleeve_corr_summary.get('corr_vrp_alt_vs_vrp_core', 1.0) if sleeve_corr_summary else 1.0
        
        sharpe_pass = sharpe_diff >= -0.02  # No material degradation (ideally improvement)
        maxdd_pass = maxdd_diff <= 0.01  # No materially worse drawdown (1% tolerance)
        corr_pass = corr_vrp_core < 0.95  # Non-redundancy check
        
        verdict = "PASS" if (sharpe_pass and maxdd_pass and corr_pass) else "INCONCLUSIVE" if (sharpe_pass and maxdd_pass) else "FAIL"
        
        with open(phase2_file, 'w') as f:
            f.write(f"# Phase-2: VRP-Alt Portfolio Integration\n")
            f.write(f"# Registered: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"baseline_profile: {args.baseline_profile}\n")
            f.write(f"variant_profile: {args.variant_profile}\n")
            ...
            if sleeve_corr_summary:
                if 'corr_vrp_alt_vs_vrp_core' in sleeve_corr_summary:
                    f.write(f"corr_vrp_alt_vs_vrp_core: {sleeve_corr_summary['corr_vrp_alt_vs_vrp_core']:.4f}\n")
                if 'corr_vrp_alt_vs_trend' in sleeve_corr_summary:
                    f.write(f"corr_vrp_alt_vs_trend: {sleeve_corr_summary['corr_vrp_alt_vs_trend']:.4f}\n")
                if 'corr_vrp_alt_vs_csmom' in sleeve_corr_summary:
                    f.write(f"corr_vrp_alt_vs_csmom: {sleeve_corr_summary['corr_vrp_alt_vs_csmom']:.4f}\n")
                if 'corr_vrp_alt_vs_vrp_convergence' in sleeve_corr_summary:
                    f.write(f"corr_vrp_alt_vs_vrp_convergence: {sleeve_corr_summary['corr_vrp_alt_vs_vrp_convergence']:.4f}\n")
            f.write(f"\n# Phase-2 Pass Criteria\n")
            f.write(f"sharpe_pass: {'PASS' if sharpe_pass else 'FAIL'} (variant >= baseline - 0.02)\n")
            f.write(f"maxdd_pass: {'PASS' if maxdd_pass else 'FAIL'} (variant <= baseline + 0.01)\n")
            f.write(f"corr_pass: {'PASS' if corr_pass else 'FAIL'} (corr(VRP-Alt, VRP-Core) < 0.95)\n")
            f.write(f"\nverdict: {verdict}\n")
            f.write(f"reason: {'All criteria passed' if verdict == 'PASS' else 'One or more criteria failed' if verdict == 'FAIL' else 'Marginal pass'}\n")
            f.write(f"\npath: {output_dir}\n")
        ...
        if sleeve_corr_summary:
            print(f"\nSleeve Correlations:")
            if 'corr_vrp_alt_vs_vrp_core' in sleeve_corr_summary:
                print(f"  corr(VRP-Alt, VRP-Core): {sleeve_corr_summary['corr_vrp_alt_vs_vrp_core']:.4f}")
            if 'corr_vrp_alt_vs_trend' in sleeve_corr_summary:
                print(f"  corr(VRP-Alt, Trend): {sleeve_corr_summary['corr_vrp_alt_vs_trend']:.4f}")
            if 'corr_vrp_alt_vs_csmom' in sleeve_corr_summary:
                print(f"  corr(VRP-Alt, CSMOM): {sleeve_corr_summary['corr_vrp_alt_vs_csmom']:.4f}")
            if 'corr_vrp_alt_vs_vrp_convergence' in sleeve_corr_summary:
                print(f"  corr(VRP-Alt, VRP-Convergence): {sleeve_corr_summary['corr_vrp_alt_vs_vrp_convergence']:.4f}")
        
        print(f"\nPhase-2 Status (VRP-Alt): {verdict}")
        print(f"Reason: {'All criteria passed' if verdict == 'PASS' else 'One or more criteria failed' if verdict == 'FAIL' else 'Marginal pass'}")
