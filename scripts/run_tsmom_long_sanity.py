"""
CLI script for running Long-Term TSMOM-252 Sign-Only Sanity Check.

This script implements a Phase-0 diagnostic for the long-term momentum idea:
- 252-day return with 21-day skip, vol-normalized using 63-day rolling volatility
- Sign-only signals, equal-weighted across assets
- No overlays, no vol targeting, no cross-sectional z-scoring

Usage:
    python scripts/run_tsmom_long_sanity.py --start 2021-01-01 --end 2025-10-31
"""
...
from src.diagnostics.tsmom_long_sanity import (
    run_tsmom_long,
    save_results,
    generate_plots
)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Long-Term TSMOM-252 Sign-Only Sanity Check",
...
Examples:
  ...
  # Custom parameters
  python scripts/run_tsmom_long_sanity.py --start 2021-01-01 --end 2025-10-31 --lookback 252 --skip_recent 21 --vol_window 63
  
  # Specific universe
  python scripts/run_tsmom_long_sanity.py --start 2021-01-01 --end 2025-10-31 --universe ES,NQ,RTY,CL,GC
  
  # Custom output directory
  python scripts/run_tsmom_long_sanity.py --start 2021-01-01 --end 2025-10-31 --output_dir reports/sanity_checks/trend/long_term/my_run
        """
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=252,
        help="Lookback period for return calculation in days (default: 252)"
    )
    parser.add_argument(
        "--skip_recent",
        type=int,
        default=21,
        help="Days to skip at the end to avoid microstructure noise (default: 21)"
    )
    parser.add_argument(
        "--vol_window",
        type=int,
        default=63,
        help="Window for volatility calculation in days (default: 63)"
    )
    parser.add_argument(
        "--universe",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory (default: reports/sanity_checks/trend/long_term/<timestamp>)"
    )
    ...
    try:
        logger.info("=" * 80)
        logger.info("LONG-TERM TSMOM-252 SIGN-ONLY SANITY CHECK")
        logger.info("=" * 80)
        logger.info(f"Start date: {args.start}")
        logger.info(f"End date: {args.end or 'latest available'}")
        logger.info(f"Lookback: {args.lookback} days")
        logger.info(f"Skip recent: {args.skip_recent} days")
        logger.info(f"Vol window: {args.vol_window} days")
        ...
        # Load continuous prices and returns
        logger.info("\n[2/4] Loading continuous prices and returns...")
        prices_cont = market.prices_cont
        returns_cont = market.returns_cont
        logger.info(f"  Prices shape: {prices_cont.shape}")
        logger.info(f"  Returns shape: {returns_cont.shape}")
        ...
        if universe:
            ...
            returns_cont = returns_cont[available]
        ...
        if args.output_dir:
            ...
            sleeve_dirs = get_sleeve_dirs("trend", "long_term")
        ...
        # Run strategy
        logger.info("\n[3/4] Computing long-term momentum strategy...")
        result = run_tsmom_long(
            prices=prices_cont,
            returns=returns_cont,
        ...
        # Print summary
        print("\n" + "=" * 80)
        print("LONG-TERM TSMOM-252 SIGN-ONLY SANITY CHECK RESULTS")
        print("=" * 80)
        ...
        if not pd.isna(sharpe):
            if sharpe >= 0.2:
                ...
                comment = "Long-term momentum idea shows positive alpha. Eligible for Phase-1."
            elif sharpe >= 0.0:
                verdict = "⚠ WEAK (0.0 <= Sharpe < 0.2)"
                comment = "Long-term momentum idea shows weak or zero alpha. May need refinement."
            else:
                verdict = "✗ FAIL (Sharpe < 0.0)"
                comment = "Long-term momentum idea shows negative alpha. Redesign required."
            print(f"\n  {verdict}")
            print(f"  {comment}")
        else:
        ...
        if latest_dir is not None:
            ...
            update_phase_index("trend", "long_term", "phase0")
