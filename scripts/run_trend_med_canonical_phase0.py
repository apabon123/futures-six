"""
CLI script for running Canonical Medium-Term (84d) TSMOM Sign-Only Sanity Check (Phase-0).

This script implements a deliberately simple, academic-style trend-following strategy
to verify that the canonical medium-term horizon (84 days) shows positive Sharpe on
our data before proceeding with the full multi-feature implementation.

Strategy (Phase-0):
- For each asset, compute 84-day return momentum (skip 10 days)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Canonical Medium-Term (84d) TSMOM Sign-Only Sanity Check (Phase-0)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic run with defaults (84d lookback, 10d skip)
  python scripts/run_trend_med_canonical_phase0.py --start 2020-01-01 --end 2025-10-31
  
  # Custom universe
  python scripts/run_trend_med_canonical_phase0.py --start 2020-01-01 --end 2025-10-31 --universe ES,NQ,RTY,CL,GC
  
  # Custom output directory
  python scripts/run_trend_med_canonical_phase0.py --start 2020-01-01 --end 2025-10-31 --output_dir reports/sanity_checks/trend/medium_canonical/custom
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory (default: reports/sanity_checks/trend/medium_canonical/archive/<timestamp>)"
    )
    
    args = parser.parse_args()
    
    # Canonical medium-term parameters
    LOOKBACK = 84  # Canonical medium-term horizon
    SKIP_RECENT = 10  # Skip recent days to avoid noise
    HORIZON_NAME = "medium_canonical_84"
    
    try:
        logger.info("=" * 80)
        logger.info("CANONICAL MEDIUM-TERM (84d) TSMOM SIGN-ONLY SANITY CHECK (Phase-0)")
        logger.info("=" * 80)
        logger.info(f"Start date: {args.start}")
        logger.info(f"End date: {args.end}")
        logger.info(f"Lookback: {LOOKBACK} days (canonical medium-term)")
        ...
        atomic_sleeve_dir = meta_sleeve_dir / "medium_canonical"
        ...
        logger.info(f"\n[3/5] Computing sign-only TSMOM for canonical medium-term (84d, skip={SKIP_RECENT}d)...")
        
        # For Phase-0, we need to manually apply the skip logic since run_sign_only_momentum
        # doesn't have a skip parameter. We'll use a custom lookback that includes the skip.
        # Effective lookback for signal = 84d return, evaluated 10 days ago
        # This means we use prices[t-10] / prices[t-10-84] - 1
        # We can approximate this by using lookback = 84 + 10 = 94 days
        # But to be more accurate, let's compute the skip manually
        
        # Compute lookback returns with skip
        logger.info(f"  Computing 84d returns with {SKIP_RECENT}d skip...")
        
        # We need: return from (t-10-84) to (t-10)
        # Method: Shift prices by skip_recent, then compute 84d return
        # This gives us: price[t-10] / price[t-10-84] - 1
        ...
        # Don't dropna here - we'll align with daily_returns and handle NaN during alignment
        
        # Compute daily returns (no shift needed)
        daily_returns = prices_cont.pct_change(fill_method=None)
        # Don't dropna here either - align first
        
        # Align returns (need common dates where both have data)
        # Use inner join to only keep dates where both have at least some valid data
        ...
        # This is important: we need history BEFORE start_date to compute lookback returns
        if args.start:
        ...
        phase_index_dir = Path("reports/phase_index/trend/medium_canonical")
        phase_index_dir.mkdir(parents=True, exist_ok=True)
        
        phase0_file = phase_index_dir / "phase0.txt"
        with open(phase0_file, 'w') as f:
            f.write(f"# Phase-0: Canonical Medium-Term (84d) Sign-Only Sanity Check\n")
        ...
        print("\n" + "=" * 80)
        print("CANONICAL MEDIUM-TERM (84d) SIGN-ONLY TSMOM SUMMARY (Phase-0)")
        print("=" * 80)
