"""
Phase-1 Diagnostics Script for Canonical Medium-Term (84d) Atomic Sleeve.

Runs two strategy profiles:
1. Standalone Canonical Medium-Term (medium_canonical_phase1)
2. Legacy Medium-Term (for comparison, using default medium_momentum config)

Saves results to reports/runs/<run_id>/ for comparison.

This validates the canonical medium-term sleeve as a standalone atomic sleeve
...
def main():
    """
    Run Phase-1 diagnostics for Canonical Medium-Term (84d) atomic sleeve.
    
    This script runs two strategy profiles:
    1. Standalone Canonical Medium-Term (medium_canonical_phase1)
    2. Legacy Medium-Term (for comparison)
    
    Results are saved to reports/runs/<run_id>/ for comparison.
    """
    parser = argparse.ArgumentParser(
        description="Run Phase-1 diagnostics for Canonical Medium-Term (84d) atomic sleeve",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic run with default run_ids
  python scripts/run_trend_med_canonical_phase1.py
  ...
  python scripts/run_trend_med_canonical_phase1.py \\
    --run_id_prefix med_canonical_phase1_v1
        """
    )
    ...
    parser.add_argument(
        "--run_id_prefix",
        type=str,
        default="med_canonical_phase1",
        help="Prefix for run identifiers (default: med_canonical_phase1)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="2020-01-01",
        help="Start date for backtest (default: 2020-01-01, first date in database)"
    )
    ...
    parser.add_argument(
        "--skip_standalone",
        action="store_true",
        help="Skip standalone canonical medium-term run"
    )
    
    parser.add_argument(
        "--skip_legacy",
        action="store_true",
        help="Skip legacy medium-term run (for comparison)"
    )
    
    args = parser.parse_args()
    
    logger.info("=" * 80)
    logger.info("CANONICAL MEDIUM-TERM (84d) PHASE-1 DIAGNOSTICS")
    logger.info("=" * 80)
    ...
    # 1. Standalone Canonical Medium-Term
    if not args.skip_standalone:
        standalone_run_id = f"{args.run_id_prefix}_standalone_{timestamp}"
        logger.info("\n[1/2] Running Standalone Canonical Medium-Term (84d)...")
        run_strategy_profile(
            profile_name="medium_canonical_phase1",
            run_id=standalone_run_id,
            start_date=args.start,
            end_date=args.end
        )
    else:
        logger.info("\n[1/2] Skipping Standalone Canonical Medium-Term")
        standalone_run_id = None
    
    # 2. Legacy Medium-Term (for comparison)
    if not args.skip_legacy:
        legacy_run_id = f"{args.run_id_prefix}_legacy_{timestamp}"
        logger.info("\n[2/2] Running Legacy Medium-Term (for comparison)...")
        # Note: This uses the existing MediumTermMomentumStrategy
        # We'll use momentum_only_v1 profile but replace long with medium-term
        # For now, we can use a custom profile or manually create one
        # We'll assume a profile "medium_legacy_phase1" exists in strategies.yaml
        run_strategy_profile(
            profile_name="medium_legacy_phase1",
            run_id=legacy_run_id,
            start_date=args.start,
            end_date=args.end
        )
    else:
        logger.info("\n[2/2] Skipping Legacy Medium-Term")
        legacy_run_id = None
    ...
    if standalone_run_id:
        logger.info(f"  Standalone Canonical Medium-Term: {standalone_run_id}")
    if legacy_run_id:
        logger.info(f"  Legacy Medium-Term: {legacy_run_id}")
    ...
    with open(phase1_file, 'w') as f:
        f.write(f"# Phase-1: Canonical Medium-Term (84d) Standalone Atomic Sleeve\n")
